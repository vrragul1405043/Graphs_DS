...
bool cycle_detection_BFS(int N, vector<vector<int>>&adjacency_list){
    vector<int>inDegree(N,0);
    queue<int>Q;
    int cnt = 0;
    for(int i=0;i<N;i++){
...
int main()
{
    ...
    vector<vector<int>>adjacency_list_cycle = {
        {1},
        {2},
        {3},
        {1}
    ...
    cout<<endl;
    if(cycle_detection_BFS(adjacency_list_cycle.size(),adjacency_list_cycle))cout<<"There exists a cycle "<<endl;
